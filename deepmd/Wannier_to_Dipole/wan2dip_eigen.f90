...
SUBROUTINE INITIALIZE
  ...
  ALLOCATE(eigenvalues(nwfc)) ! Wannier eigenvalues
  ...
  OPEN(unit = 7,file = 'eigenvalues.raw')
  OPEN(unit = 8,file = 'eigenvalues_ordered.raw')

END SUBROUTINE INITIALIZE

SUBROUTINE READ_RAW_WFC 
  !Read DPMD raw file
  USE wannier, only : wfc, nwfc, eigenvalues
  IMPLICIT NONE
  INTEGER                    :: i, j
 
  !Read Wannier centers
  READ(4,*) ( ( wfc(i, j), i=1,3 ), j=1,nwfc )
  READ(7,*) ( eigenvalues(j), j=1,nwfc )
...
SUBROUTINE ORDERED_WFC
  ...
  IMPLICIT NONE
  REAL*8                     :: wfctmp(3,2*nwfc), wfc_atom(3,nwfc)
  REAL*8                     :: dipole(3,natoms), eigen(2*nwfc), eigen_(nwfc) 
  INTEGER                    :: i, iat, idip
  INTEGER                    :: iwf
  ...
  DO iat = 1,natoms
    ...
    IF ( nwf_atom(atype(iat))>0 ) THEN 
      ...
      CALL NEAREST_WFCs( iat, wfc_atom, eigen_, nwf_atom(atype(iat)) )
  ...
  CALL Print_Wannier_Eigenvalues(eigen,iwf-1)
...
SUBROUTINE PRINT_Wannier_Eigenvalues(eigen,n)
  IMPLICIT NONE
  INTEGER,INTENT(IN)         :: n 
  REAL*8, INTENT(IN)         :: eigen(n)
  INTEGER                    :: i
  WRITE(8, fmt='(*(E16.7E4,3X))'), ( eigen(i), i=1,n )
END SUBROUTINE

SUBROUTINE Print_Dipole(dipole,n,ndip)
  IMPLICIT NONE
  INTEGER,INTENT(IN)         :: n, ndip
  REAL*8, INTENT(IN)         :: dipole(3,n)
  INTEGER                    :: i
  WRITE(6, fmt='(*(E16.7E4,3X))'), ( dipole(1,i), dipole(2,i), dipole(3,i), i=1,ndip )
END SUBROUTINE

SUBROUTINE NEAREST_WFCs( iat, wfcs, eigen, n )
  ...
  DO iwf=1,n
    ...
    eigen(iwf)=eigenvalues(ind_wf(iwf))
  END DO
