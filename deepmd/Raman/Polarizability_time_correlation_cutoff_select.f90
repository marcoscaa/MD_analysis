...
MODULE correlation 
  ...
  DOUBLE PRECISION, PARAMETER              :: rcut=4.d0
...
SUBROUTINE INITIALIZE
  ...
  READ(1,*) natoms, nframes, nequil, nwater, ncorr, dt, tcorr
  READ(1,*) mean_pol
  ALLOCATE(ind_corr(ncorr))
  ind_corr = (/ ( i, i=1,ncorr ) /)
  IF ( ncorr .lt. nwater ) READ(1,*) ( ind_corr(i) , i=1,ncorr )
  CLOSE(1)
  ...
  OPEN(unit = 4,file = 'polarizability.raw')
...
SUBROUTINE POLAR_CORR
  ...
  USE correlation, ONLY : isopolar, anisopolar, iso_intra_corr, iso_inter_corr, & 
                         aniso_intra_corr, aniso_inter_corr, tcorr, ind_corr, ncorr
  ...
  INTEGER                    :: frame1, frame2, i_at, j_at, nt0, i
...
!$omp parallel do private(i_at) reduction(+:sumiso_inter,sumiso_intra,sumaniso_intra,sumaniso_inter)
      DO i = 1,ncorr
    ...
    IF (is_water_oxygen(iat)) THEN
      ...
      DO jat = iat,natoms
        ...
        IF (is_water_oxygen(jat)) THEN
          ...
          IF (iat==jat) THEN 
            !Simpler to separate intra and intermolecular terms
