...
!$omp parallel do reduction(+:sumiso_inter,sumiso_intra,sumaniso_intra,sumaniso_inter)
    DO i_at = 1,nwater
      ...
      sumiso_intra = sumiso_intra + &
          isopolar(i_at,1) * isopolar(i_at,frame1) 

      sumaniso_intra = sumaniso_intra + &
          tracematmul(anisopolar(:,:,i_at,1), anisopolar(:,:,i_at,frame1))

      DO j_at = i_at+1, nwater

        IF ( within_cutoff(j_at, i_at, 1) ) THEN

          sumiso_inter = sumiso_inter + &
              isopolar(i_at,1) * isopolar(j_at,frame1) + &
              isopolar(j_at,1) * isopolar(i_at,frame1) 

          sumaniso_inter = sumaniso_inter + &
              tracematmul(anisopolar(:,:,i_at,1), anisopolar(:,:,j_at,frame1)) + &
              tracematmul(anisopolar(:,:,j_at,1), anisopolar(:,:,i_at,frame1)) 
...
!$omp end parallel do
    ...
    iso_intra_corr(frame1)   = iso_intra_corr(frame1)   + sumiso_intra
    iso_inter_corr(frame1)   = iso_inter_corr(frame1)   + sumiso_inter
    aniso_intra_corr(frame1) = aniso_intra_corr(frame1) + sumaniso_intra
    aniso_inter_corr(frame1) = aniso_inter_corr(frame1) + sumaniso_inter
