...
SUBROUTINE MAKE_HISTOGRAM(lsi_histogram,lsi_op)
  ...
  REAL*8, PARAMETER :: max_lsi=1.d0
    
  ind = int(nhist*lsi_op/max_lsi) + 1
...
SUBROUTINE PRINT_RESULTS
  USE parameters, ONLY : nhist, lsi_histogram
  IMPLICIT NONE
  INTEGER                    :: ihist
  ...
  DO ihist = 1, nhist
    ...
    WRITE(2,fmt='(F12.8,3X,I10)') float(2*ihist-1)/float(nhist)/2.d0, lsi_histogram(ihist) 
