...
SUBROUTINE INITIALIZE
  USE parameters, only : natoms,nframes,nequil,stride,&
                         nhist, pos, atype 
  USE histogram
  IMPLICIT NONE
  INTEGER                    :: i
  ...
  ALLOCATE(hist_angle(nhist,maxcoord));hist_angle=0
...
SUBROUTINE MAKE_HISTOGRAM
  USE parameters, ONLY : natoms, atype, nhist
    ...
    IF (atype(iat)==atype1) THEN
      ...
      DO n1 = 1,nbonds
        DO n2 = n1+1,nbonds
          ...
          hist_angle(ind,nbonds) = hist_angle(ind,nbonds) + 1
        END DO
      END DO
    END IF
  END DO
...
SUBROUTINE PRINT_RESULTS
  ...
  USE histogram, ONLY : hist_angle, maxcoord
  IMPLICIT NONE
  INTEGER                    :: ic, n
  REAL*8                     :: bin
  
  OPEN(unit = 2,file = "hist_angle_coord.dat")
  write(2,*) '# ', 'cos(Angle)', 'Count(1) ... Count(ncoord)'

  bin = 2./real(nhist)

  DO ic = 1, nhist
    WRITE(2,fmt = "(F12.8, *(3X,I10))"), bin*(real(ic)-0.5) - 1.0,&
                                   ( hist_angle(ic,n), n=1,maxcoord ) 
  END DO
...
!$omp parallel do private(normd,d) reduction(+:nbonds)
  DO iat2 = 1, natoms
