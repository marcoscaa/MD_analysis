...
SUBROUTINE INITIALIZE
  USE parameters, ONLY : natoms,pos, atype, &
                         ...
                         ind_rdf, maxr, stride, local_frame
  IMPLICIT NONE
  INTEGER                    :: i
  CHARACTER(100)             :: pos_file, index_file, local
  ...
  READ(*,*) ind_rdf,local !Atomic species to be computed by RDF
                          !0 means all species
...
SUBROUTINE COMPUTE_RDF
  USE parameters, ONLY : natoms, pos, ind_rdf, box, fixed_coord, &
                         nhist, H, atype, maxr, local_frame
  IMPLICIT NONE
  INTEGER                     :: iat, iat2
  INTEGER                     :: ind1,ind2 
  ...
  REAL*8                      :: v1(2),v2(2)
    ...
    DO iat2 = 1, natoms
      ...
      IF ((( atype(iat2) == ind_rdf(2)) .or. ( ind_rdf(2) == 0 ))) THEN
        ...
        if (local_frame) d = MATMUL(rotmat,d)
