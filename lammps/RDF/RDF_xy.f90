...
SUBROUTINE COMPUTE_RDF
  USE parameters, ONLY : natoms, pos, ind_rdf, box, fixed_coord, &
                         nhist, gofr, atype, maxr
  IMPLICIT NONE
  INTEGER                     :: iat, iat2
  INTEGER                     :: n_coord 
  INTEGER                     :: ind, gofr_tmp(nhist)
  INTEGER                     :: n
  REAL*8                      :: Dist, d, central_atom(3)
...
SUBROUTINE PRINT_RESULTS
  USE parameters, ONLY : nhist, gofr, maxr, number_density, n, &
  ...
  DO ihist = 1, nhist
    ...
    vol = Pi * ( (float(ihist)*bin)**2 - ((float(ihist)-1)*bin)**2 ) * box(3)
    aver = float(gofr(ihist)) / vol / float(n) 
