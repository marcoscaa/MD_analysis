MODULE parameters
  ...
  DOUBLE PRECISION, ALLOCATABLE                :: q_norm(:), form_h(:,:)
...
SUBROUTINE Q_INDEX_TABLE
  USE parameters, ONLY : q_vector, q_norm, nq, Pi, qmax, box, nk, &
                         Stmp, q_norm_d, ntype, form_d, form_h
  IMPLICIT NONE
  INTEGER                    :: i,j,k,c
...
SUBROUTINE COMPUTE_ATOMIC_FORM_FACTORS
  USE parameters, ONLY : box, form_d, ntype, nq, q_norm_d, form_h
  USE cudafor
  IMPLICIT NONE
...
SUBROUTINE CUDA_COMPUTE_SofK
  USE parameters, ONLY : natoms, pos, nhist, q_norm, qmax, &
                         ...
                         form_h, q_vector, nq, counter, Pi
  USE cudafor
  IMPLICIT NONE
  INTEGER                     :: iat, jat
  INTEGER                     :: qi, qind, n1, n2
  DOUBLE PRECISION            :: f1,f2,g1,g2,newS
  ...
  DO qi = 1,nq 
    ...
    if (qind.le.nhist) then
      f1=0.0;f2=0.0;g1=0.0;g2=0.0
      !n1=0;n2=0
      !$cuf kernel do(1)<<<*,*>>>
      DO iat = 1, natoms
        dp = sum(q_vector(:,qi) * pos_d(:,iat))
        if (atype_d(iat)==1) then
          f1 = f1 + cos(dp)
          f2 = f2 + sin(dp)
          !n1 = n1+1
        elseif (atype_d(iat)==2) then
          g1 = g1 + cos(dp)
          g2 = g2 + sin(dp)
          !n2 = n2+1
        endif
      END DO
      newS = ( f1**2 + f2**2 )*form_h(qi,1)**2 &
           + ( g1**2 + g2**2 ) * form_h(qi,2)**2 &
           + 2.d0 * ( f1*g1 + f2*g2 ) * form_h(qi,1)*form_h(qi,2)
      newS = newS / dble(natoms) - (1./3.)*form_h(qi,1)**2 - (2./3.)*form_h(qi,2)**2
      newS = newS / ( (1./3.)*form_h(qi,1) + (2./3.)*form_h(qi,2) ) **2 
      S(qind) = S(qind) + newS
...
SUBROUTINE PRINT_RESULTS
  USE parameters, ONLY : nhist, S, qmax, counter, natoms
  IMPLICIT NONE
