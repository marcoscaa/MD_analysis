MODULE histogram 
  IMPLICIT NONE 
  REAL*8,ALLOCATABLE                       :: hist_dist(:,:),hist_angle(:,:)
...
SUBROUTINE INITIALIZE
  ...
  ALLOCATE(hist_angle(2,nhist)); hist_angle=0.0
  ALLOCATE(hist_dist(2,nhist)); hist_dist=0.0
          ...
          IF ( (ind_H(1) /= 0) .and. (ind_H(2) /= 0 )) THEN
            ...
            IF (d_OOw < 3.5) THEN 
              IF (ang_OHO(1) > 0.8660) THEN
                CALL ADD_TO_HISTOGRAM(j,ind_H(1),ang_OHO(1))
              ELSE IF (ang_OHO(2) > 0.8660) THEN
                CALL ADD_TO_HISTOGRAM(j,ind_H(2),ang_OHO(2))
              END IF
...
SUBROUTINE ADD_TO_HISTOGRAM(ind1,ind2,OHO_angle)
  USE parameters, only : nhist
  USE histogram, only : max_hblen,hist_angle,hist_dist
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: ind1, ind2
  REAL*8, INTENT(IN) :: OHO_angle
  REAL*8 :: OH(3), d
  INTEGER :: hist_index

  CALL DIST_UNIT_VECTOR(ind1,ind2,OH,d)
  hist_index=int(((OHO_angle)-0.8660)/0.134*nhist) + 1
  hist_angle(1,hist_index) = hist_angle(1,hist_index) + OH(3)**2
  hist_angle(2,hist_index) = hist_angle(2,hist_index) + (1-OH(3)**2)
  hist_index=int(d/max_hblen*nhist) + 1
  hist_dist(1,hist_index) = hist_dist(1,hist_index) + OH(3)**2
  hist_dist(2,hist_index) = hist_dist(2,hist_index) + (1-OH(3)**2)
...
SUBROUTINE PRINT_RESULTS
  ...
  OPEN(unit = 2, file = "HB_angle_decomposed.dat")
  OPEN(unit = 3, file = "HB_length_decomposed.dat")

  dl=max_hblen/float(nhist)
  hist_dist = hist_dist/sum(hist_dist)

  WRITE(2,fmt="(A35)") "# OHO_Angle(cos) P(axial) P(radial)"
  WRITE(3,fmt="(A40)") "# HB_length(angstrom) P(axial) P(radial)"

  DO ihist=1,nhist
    !WRITE(2,fmt='(F12.8,3X,I10)') float(ihist-1)*0.5235/float(nhist)*57.3, hist_angle(ihist)
    WRITE(2,fmt='(F12.8,2(3X,E12.5))') (float(ihist-1))*0.134/float(nhist) + 0.8660, &
            hist_angle(1,ihist), hist_angle(2,ihist)
    WRITE(3,fmt='(F12.8,2(3X,E12.5))') float(ihist-1)*dl, &
            hist_dist(1,ihist), hist_dist(2,ihist)
  END DO
