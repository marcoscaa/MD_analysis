...
SUBROUTINE INITIALIZE
  USE parameters, only : natoms,nframes,nequil,nhist, pos, atype,box 
  USE histogram
  IMPLICIT NONE
  INTEGER                    :: i
...
SUBROUTINE MAKE_HISTOGRAM
  ...
  DO iat = 1, natoms
    IF (trim(atype(iat))==trim(atype1)) THEN
...
SUBROUTINE CONNECT_NEAREST_NEIGHBOHRS( iat,bonds,nbonds )
  ...
  DO iat2 = 1, natoms
    IF ( (iat2.ne.iat).and.(trim(atype(iat2)).eq.trim(atype2)) ) THEN
