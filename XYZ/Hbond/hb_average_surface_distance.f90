...
INTEGER FUNCTION get_z_index(iatom)
  USE parameters, only : nhist, box, pos
  INTEGER, INTENT(IN) :: iatom 
  INTEGER             :: indz 
  REAL*8              :: posz,boxz

  boxz=box(3,3)
  posz = pos(3,iatom) - nint(pos(3,iatom)/boxz)*boxz
  posz = posz + boxz/2.
...
SUBROUTINE PRINT_RESULTS
  ...
  DO ihist=1,nhist
    ...
    WRITE(2,fmt='(F12.8,2(3X,F12.8))') dz*float(ihist-1), &
  ...
  DO ihist=1,nhist
    WRITE(3,fmt='(F12.8,2(3X,F12.8))') dz*float(ihist-1), &
