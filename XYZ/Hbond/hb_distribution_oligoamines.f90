...
SUBROUTINE ANALYSIS
  ...
  DO i=1,natoms
    ...
    END IF
    IF ( trim(atype(i)) == trim(ind_hb(2)) ) THEN
...
SUBROUTINE PRINT_RESULTS
  USE histogram, ONLY : hist_hb_peratom, hb_length
  IMPLICIT NONE
  INTEGER :: ihist
  ...
  WRITE(2, *) "# N_Hbonds Count(Donated, species 1) Count(Accepted, species 2)"
  WRITE(3, *) "# HBond_Length_Donated(A) Count(species 1)"
