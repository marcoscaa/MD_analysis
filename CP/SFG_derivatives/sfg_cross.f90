...
MODULE wannier
  ...
  DOUBLE PRECISION, ALLOCATABLE            :: cforce(:,:,:) !off-diagonal  
...
SUBROUTINE INITIALIZE
  ...
  CHARACTER(100)             :: wfc_field_file, cforce_file 

  CALL getarg(1, pos_file)
  CALL getarg(2, forces_file)
  CALL getarg(3, cforce_file)
  CALL getarg(4, wfc_file)
  ...
  ALLOCATE(cforce(natoms,12,3)); cforce=0
  ...
  OPEN(unit = 3,file = cforce_file)
  OPEN(unit = 4,file = wfc_file)
  
END SUBROUTINE INITIALIZE

SUBROUTINE READ_POS_FORCES
  USE wannier, ONLY : forces, cforce 
  ...
  DO iat = 1, natoms
    ...
    !Field components: xy, -xy, x-y -x-y, xz, -xz, x-z, -x-z, yz, -yz, y-z, -y-z  
    READ(3, *), ( ( cforce(iat,ifield,ipol), ipol=1,3 ), ifield=1,12 )
  END DO
...
SUBROUTINE BOND_FRAME_DERIVATIVE (frame)
  ...
  INTEGER                    :: n_hydrogens
  INTEGER                    :: ind_layer_pf, j, k
  LOGICAL                    :: is_oxygen
  DOUBLE PRECISION           :: oh(2,3)
  DOUBLE PRECISION           :: der1(3), der2(3,3)
  ...
  DO i = 1,natoms
    ...
    IF ( is_oxygen(i) ) THEN
      ...
      DO ih = 1,n_hydrogens
        ...
        WRITE(50,fmt="(i2,4(3X,E12.5))"), ind_layer_pf(i,n_hydrogens), der1
        WRITE(51,fmt="(i2,7(3X,E12.5))"), ind_layer_pf(i,n_hydrogens), &
                                          ( ( der2(j,k), k=1,3 ), j=1,3 )
...
SUBROUTINE dF_dEps( iO, iH, rotmat, induced_field, der1 )
  ...
  USE wannier, ONLY : forces, efield, cforce
  IMPLICIT NONE
...
SUBROUTINE d2F_dEps2( iO, iH, rotmat, induced_field, der2 )
  ...
  DOUBLE PRECISION, INTENT(IN)         :: rotmat(3,3)
  DOUBLE PRECISION, INTENT(IN)         :: induced_field(3,3)
  DOUBLE PRECISION, INTENT(INOUT)      :: der2(3,3)
  DOUBLE PRECISION                     :: dF(3,3), dFtmp(3)
  INTEGER                              :: ipol, ifield1, ifield2, i

  DO ifield1 = 1,3
    DO ifield2 = ifield1,3

      DO ipol = 1,3

        IF (ifield1.eq.ifield2) THEN

          i=2*ifield1
          dFtmp(ipol) = forces(iH,i  ,ipol) - forces(iO,i  ,ipol) &
                      + forces(iH,i+1,ipol) - forces(iO,i+1,ipol) &
                      - 2*forces(iH,1,ipol) + 2*forces(iO,1,ipol)

        ELSE

          i=4*(ifield2+ifield1-3)+1
          dFtmp(ipol) = cforce(iH,i  ,ipol) - cforce(iO,i  ,ipol) &
                      - cforce(iH,i+1,ipol) + cforce(iO,i+1,ipol) &
                      - cforce(iH,i+2,ipol) + cforce(iO,i+2,ipol) &
                      + cforce(iH,i+3,ipol) - cforce(iO,i+3,ipol)
          dFtmp(ipol) = dFtmp(ipol) / 4.d0
      ...
      IF ( ifield1.eq.ifield2 ) THEN
        !Only the component of the force along the OH bond is considered
        dF(ifield1,ifield2) = sum( rotmat(3,:) * dFtmp ) / &
                      ( efield(ifield1) + induced_field(ifield1,ifield1) )**2
      ELSE
        dF(ifield1,ifield2) = sum( rotmat(3,:) * dFtmp ) / &
                      ( ( efield(ifield1) + induced_field(ifield1,ifield1) ) * &
                      ( efield(ifield2) + induced_field(ifield2,ifield2)) )
        dF(ifield2,ifield1) = df(ifield1,ifield2)
      END IF
  ...
  !Convert derivative from lab frame to the bond frame
  der2=MATMUL( rotmat, dF )
  der2=MATMUL( der2, TRANSPOSE(rotmat) )
