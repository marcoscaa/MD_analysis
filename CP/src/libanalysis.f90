...
SUBROUTINE REMOVE_EQUIL_WANN
  USE parameters, ONLY : nwann, nequil
  IMPLICIT NONE
  INTEGER                    :: iframe

  DO iframe = 1,nequil*(nwann+9)
    READ(3,*)
  END DO

END SUBROUTINE REMOVE_EQUIL_WANN
...
SUBROUTINE NATOMS_BUFFER(nat_buff) 
  !Needed if dumping fewer atoms (in Lammps)
  !than the total number of atoms used in the simulation
  USE parameters, ONLY : natoms
  IMPLICIT NONE
  INTEGER                    :: iat, ind, nat_buff
  ...
  DO iat = 1, natoms
    READ(1,*) ind
    if (ind>nat_buff) nat_buff=ind
  END DO

  REWIND(1)

END SUBROUTINE NATOMS_BUFFER

SUBROUTINE READ_VELOCITIES 
  !Read LAMMPS custom velocities
  USE parameters, ONLY : vel, atype
  IMPLICIT NONE
  INTEGER                    :: iat, ind, atp, nat
...
SUBROUTINE READ_ATOM_REDUCED 
  !Read LAMMPS atom file
  USE parameters, ONLY : pos, atype, box
  IMPLICIT NONE
  INTEGER                    :: iat, ind, atp, nat
...
SUBROUTINE READ_ATOM_REDUCED_IO(iostatus) 
  !Read LAMMPS atom file
  USE parameters, ONLY : pos, natoms, atype, box
  IMPLICIT NONE
...
END SUBROUTINE READ_ATOM_REDUCED_IO
...
SUBROUTINE READ_WANNIER 
  !Read LAMMPS atom file
  USE parameters, ONLY : wannier, nwann, atype_wann
  IMPLICIT NONE
  INTEGER                    :: iat, ind, atp
  ...
  DO iat = 1, nwann
    READ(3,*) ind, atp, wannier(1,ind), wannier(2,ind), wannier(3,ind)
  END DO
...
SUBROUTINE DISTANCE_VECTOR_IND( ind1, ind2, d ) 
  ...
  DO ipol = 1,3
    d(ipol) = pos(ipol,ind1) - pos(ipol,ind2)
    d(ipol) = d(ipol) - nint(d(ipol)/box(ipol))*box(ipol)
  END DO
...
SUBROUTINE DEFINE_MOLECULE
  USE parameters, only : atype, natoms, rcut, index_bonded, molid
  IMPLICIT NONE
  INTEGER                    :: iat, jat, imol,c
  INTEGER                    :: list_mol(natoms,2)
  INTEGER, PARAMETER         :: maxneighbors=10
  REAL*8                     :: Dist, r

  molid=0
  index_bonded=0

  !Define bonded atoms first
  DO iat=1,natoms
    c=1
    DO jat=1,natoms
      r=Dist(iat,jat)
      if ((iat.ne.jat).and.(r <= rcut(atype(iat),atype(jat)) )) then
        index_bonded(iat,c)=jat
        c=c+1
      end if
    END DO
  END DO

  !Now loop over the bonds to find connected atoms
  imol=1
  DO iat=1,natoms
    IF (molid(iat)==0) THEN
      CALL CONNECT_ATOMS(imol,iat)
      imol=imol+1
    END IF
  END DO

END SUBROUTINE DEFINE_MOLECULE

RECURSIVE SUBROUTINE CONNECT_ATOMS(imol,iat)
  USE parameters, ONLY : natoms, atype, molid, index_bonded
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: iat, imol
  INTEGER :: jat, nid

  DO jat=1,10
    molid(iat)=imol
    nid=index_bonded(iat,jat)
    if (nid.ne.0) then
      if (molid(nid)==0) then
        CALL CONNECT_ATOMS(imol,index_bonded(iat,jat))
      end if
    end if
  END DO

END SUBROUTINE CONNECT_ATOMS
...
DOUBLE PRECISION FUNCTION DistXY(ind1,ind2)
  ! Distance between two points including pbc
  USE parameters, ONLY : pos, box
  IMPLICIT NONE
  DOUBLE PRECISION                     :: xyz(3)
...
LOGICAL FUNCTION is_hydrogen(ind)
  USE parameters, ONLY : atype, atypeH !ind_atom
  IMPLICIT NONE
  INTEGER, INTENT(IN)        :: ind
  
  !IF ( ind_atom(ind) == 2 ) THEN
  IF ( atype(ind) == atypeH ) THEN
    is_hydrogen = .true.
  ELSE
    is_hydrogen = .false.
  END IF
