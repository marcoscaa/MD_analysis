...
MODULE parameters
  ...
  INTEGER, DIMENSION(:,:), ALLOCATABLE     :: gem_filter
  ...
  INTEGER                                  :: natoms, natO
  INTEGER,PARAMETER                        :: nframes = 1000, nbins = 3
...
SUBROUTINE INITIALIZE
  ...
  ALLOCATE(gem_filter(nframes,natO)); gem_filter = 0.

  histN = 0

  OPEN(unit = 1,file = file_name)
 
  OPEN(unit = 2,file = "Ic.filter")
...
SUBROUTINE READ_FRAME (frame)
  ...
  !READ the filter file generated by octave (GeM)
  READ(2, fmt='(19481(i2,1X))'), gem_filter(frame,:)
...
SUBROUTINE COARSE_GRAIN (frame)
  ...
  INTEGER                    :: i, j, frame
  REAL*8                     :: z, z_pbc
  REAL*8                     :: bin_f(nbins+1) 

  !Initializing the coarse graining function
  coarse(frame,:,:) = 0
  !HARD CODED
  bin_f = (/ 0.65, 1.556, 2.463, 3.37 /)
  ...
  DO j = 1, natO
    ...
    DO i = 1, nbins
      ...
      IF ( ( z >= bin_f(i) ) .and. ( z < bin_f(i+1) ) .and. ( gem_filter(frame,j) == 1 ) ) THEN
        coarse(frame,i,j) = 1
        EXIT
      ENDIF
    ENDDO
